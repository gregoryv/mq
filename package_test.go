...
func TestFixed(t *testing.T) {
	cases := []struct {
		...
		{Fixed(PUBLISH | QoS2 | RETAIN), "PUBLISH -2-r"},
		{Fixed(PUBLISH | QoS1 | QoS2), "PUBLISH -!!-"},
		{Fixed(PUBLISH | DUP | QoS2), "PUBLISH d2--"},
		{Fixed(PUBLISH | QoS1), "PUBLISH --1-"},
		{Fixed(CONNECT), "CONNECT ----"},
	}
	for _, c := range cases {
